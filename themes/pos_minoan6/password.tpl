...
{if isset($confirmation) && $confirmation == 3}    
    <h1 class="page-subheading">Activaci&oacute;n de tu Cuenta</h1>
{else}    
...
<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
	<fieldset>
		...
		<p class="submit">
                    <button type="submit" class="btn btn-default button button-medium"><span>{*l s='Retrieve Password'*}Establecer Contrase&ntilde;a<i class="icon-chevron-right right"></i></span></button>
		</p>
	</fieldset>
</form>
{/if}
</div>
<ul class="clearfix footer_links" style="width: 100%;">
